...
def get_linkedin_ad_analytics():
    try:
        ...
        linkedin_url = (
            ...
            f'&dateRange=(start:(year:2025,month:05,day:29))'
            f'&accounts={accounts_param}'
            f'&campaigns=List(urn%3Ali%3AsponsoredCampaign%3A329582336)'
...
def save_linkedin_ad_analytics():
    ...
    for i in range(0,len(data)):
        ...
        save_country_data(targeting_criteria.get("locations", []), creative)
        save_jobtitle_data(targeting_criteria.get("titles", []), creative)
        save_seniority_data(targeting_criteria.get("seniorities", []), creative)
        save_industry_data(targeting_criteria.get("industries", []), creative)
        save_staff_range_data(targeting_criteria.get("staffCountRanges", []), creative)
        AdAnalyticsLinkedin.objects.create(
            creative=creative,
            clicks=clicks,
...
def save_country_data(data, creative_instance):
    auth = get_auth_token()
...
def save_jobtitle_data(data, creative_instance):
    auth = get_auth_token()
...
def save_seniority_data(data, creative_instance):
    auth = get_auth_token()
    ...
    for item in data:
        ...
        if seniority_name:
            Seniority.objects.get_or_create(
                creative=creative_instance,
...
def save_industry_data(data, creative_instance):
    auth = get_auth_token()
...
def save_staff_range_data(data, creative_instance):
    for urn in data:
        match = re.search(r'\((\d+),(\d+)\)', urn)
        if match:
            min_val = int(match.group(1))
            max_val = int(match.group(2))
            StaffCountRange.objects.get_or_create(
                creative = creative_instance,
                min_value = min_val,
                max_value = max_val
            )